...
public class Share_cike_modelimpl implements Home_model{


    private Base_persenter persenter;
    public Share_cike_modelimpl(Base_persenter persenter) {
        this.persenter = persenter;
    }

    @Override
    public void getData(String ...indexStr) {
        String url="http://api.duckr.cn/api/v6/photo/list/flow/";
        ...
        x.http().post(entity, new Callback.CacheCallback<String>() {
            ...
            public void onSuccess(String result) {
                LogUtils.out("网络999此刻","结果");
                Gson gson=new Gson();
                CiKe_data yaoYue = gson.fromJson(result, CiKe_data.class);
                persenter.getData(yaoYue.getData());

            }

            @Override
            public void onError(Throwable ex, boolean isOnCallback) {
                LogUtils.out("网络999此刻","错误信息"+ex);
            }

            @Override
            public void onCancelled(CancelledException cex) {
                LogUtils.out("网络999此刻","错误信息"+cex);
            }
